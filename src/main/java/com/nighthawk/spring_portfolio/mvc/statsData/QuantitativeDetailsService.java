...
@Service
@Transactional
public class QuantitativeDetailsService implements UserDetailsService {  // "implements" ties ModelRepo to Spring Security
    // Encapsulate many object into a single Bean (Person, Roles, and Scrum)
    @Autowired  // Inject PersonJpaRepository
    private QuantitativeJpaRepository quantitativeJpaRepository;
    ...
    public void addQuantitativeToUser(String personEmail, Long quantitativeId) { // by passing in the two strings you are giving the class that certain leader
        Person person = personJpaRepository.findByEmail(personEmail);
        if (person != null) {   // verify person
            Optional<Quantitative> quantitative = quantitativeJpaRepository.findById(quantitativeId);
            if (quantitative != null) { // verify role
                person.getQuantitatives().add(quantitative.get()); 
            }
        }
    }
}